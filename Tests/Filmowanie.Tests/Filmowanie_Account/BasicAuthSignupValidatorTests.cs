...
public sealed class BasicAuthSignupValidatorTests
{
    private readonly BasicAuthSignupValidator _sut;
    private readonly IFixture _fixture;
    private const int MinimalPasswordLength = 7;

    public BasicAuthSignupValidatorTests()
    {
        _sut = new BasicAuthSignupValidator();
    ...
    [InlineData("abc")]
    [InlineData("123abc")]
    public void ValidatePassword_WhenPasswordIsTooShort_ShouldHaveValidationError(string shortPassword)
    {
        // Arrange
        var dto = new BasicAuthLoginDTO("mr.bean@atkinson.com", shortPassword);
        ...
        result.IsValid.Should().BeFalse();
        result.Errors.Should().Contain(x => x.PropertyName == nameof(BasicAuthLoginDTO.Password) 
            && x.ErrorMessage == $"{nameof(BasicAuthLoginDTO.Password)} must have {MinimalPasswordLength} characters or more");
    }
    ...
    {
        // Act
        var canHandle = _sut.CanHandle<BasicAuthLoginDTO>(KeyedServices.SignUpBasicAuth, out var validator);

        // Assert
        canHandle.Should().BeTrue();
        validator.Should().NotBeNull();
        validator.Should().BeOfType<BasicAuthSignupValidator>();
    }
    ...
    {
        // Act
        var canHandle = _sut.CanHandle<string>(KeyedServices.SignUpBasicAuth, out var validator);
